Feature: Calculate average rating for a restaurant
  ...
  Scenario: Calculate average rating when there are no reviews
    Given a Restaurant "La Piazza" managed by "manager_1"
  ...
  Scenario: Calculate average rating with multiple reviews same person
    ...
    And a user "john_doe" adds a review with food rating 5.0, service rating 4.5, ambiance rating 4.5, and overall rating 5.0
    And another user "jane_doe" adds a review with food rating 4.5, service rating 4.0, ambiance rating 4.5, and overall rating 4.5
    When the average rating is calculated
  ...
  Scenario: Calculate average rating when all reviews have identical ratings different people
    ...
    And a user "emma_smith" adds a review with food rating 3.0, service rating 3.0, ambiance rating 3.0, and overall rating 3.0
    And another user "mike_jones" adds a review with food rating 3.0, service rating 3.0, ambiance rating 3.0, and overall rating 3.0
    When the average rating is calculated
