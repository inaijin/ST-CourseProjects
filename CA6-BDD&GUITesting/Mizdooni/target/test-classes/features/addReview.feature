Feature: Add reviews to a restaurant
  ...
  Scenario: Add one review when no other reviews exist
    Given a restaurant "Perperok" managed by user "manager_1"
    When a user "mamad" adds a review with the comment "Great food!" and rating 5
  ...
  Scenario: Add two reviews by the same user, where the second review overrides the first
    ...
    And a user "hesam" adds a review with the comment "Nice ambiance" and rating 4
  ...
  Scenario: Add two reviews by different users, both reviews should be present
    ...
    And a user "mamad" adds a review with the comment "Tasty BBQ" and rating 5
    When another user "ali" adds a review with the comment "Loved the ribs!" and rating 4
